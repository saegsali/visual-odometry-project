...
def test_normalize_points2D():
    ...
    assert np.allclose(
        np.sqrt(np.mean(np.sum(normalized_points**2, axis=-1))), np.sqrt(2)
    )
...
def test_normalize_points3D():
    ...
    assert np.allclose(
        np.sqrt(np.mean(np.sum(normalized_points**2, axis=-1))), np.sqrt(3)
    )
...
def test_normalize_points_complex():
    points = rng.normal(-3, 10, size=(1000, 2))
    normalized_points, T = normalize_points(points)
