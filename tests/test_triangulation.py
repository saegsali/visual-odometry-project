...
def test_find_fundamental_matrix(
    ...
    landmarks = rng.uniform(-2, 2, size=(N, 3))
    ...
    points1 = (projection_camera1.reshape(1, 3, 4) @ landmarks_hom.T).T.reshape(-1, 3)
    points2 = (projection_camera2.reshape(1, 3, 4) @ landmarks_hom.T).T.reshape(-1, 3)

    points1 = to_cartesian_coordinates(points1)
    points2 = to_cartesian_coordinates(points2)

    F = triangulator.find_fundamental_matrix(points1, points2, is_normalized=True)
...
def test_triangulate_normalize():
    pass
